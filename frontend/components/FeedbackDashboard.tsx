                        ...
                        {stats.rows.map((row: any, i: number) => (
                            <div key={i} className="p-6 hover:bg-slate-50 transition-colors">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className="text-xs text-slate-400 font-medium">{new Date(row.create_time).toLocaleDateString()}</span>
                                </div>
                                {row.comment ? (
                                ...
                                {/* If we had email subject, we would show it here. Backend currently only returns rating/time in stats. */}
                            </div>
                        ))}
                    </div>
                )}
