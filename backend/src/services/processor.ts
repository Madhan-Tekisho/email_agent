...
export const processSingleEmail = async (email: any) => {
    try {
        ...
        if (duplicates && duplicates.length > 0) {
            ...
            if (email.id) await gmailService.markAsRead(email.id);
            return;
        }
        ...
        if (ignore) {
            ...
            return; // SKIP ALL FURTHER PROCESSING - no DB insert, no reply
        }
        ...
        if (priority?.toLowerCase() === 'high') {
            ...
            if (confidence < 50) {
                ...
                await supabase
                    .from('emails')
                    .update({
                        status: 'needs_review', // CHANGED: enable SLA tracking
